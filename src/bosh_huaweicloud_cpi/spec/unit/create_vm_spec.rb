...
describe Bosh::HuaweiCloud::Cloud, 'create_vm' do
  ...
  let(:server_groups) { instance_double(Bosh::HuaweiCloud::ServerGroups) }

  before(:each) do
    @registry = mock_registry
    Bosh::Clouds::Config.configure(double('config', uuid: 'director-uuid'))
    allow(Bosh::HuaweiCloud::ServerGroups).to receive(:new).and_return(server_groups)
  ...
  context 'with manual network' do
    ...
    it 'calls NetworkConfigurator#prepare and NetworkConfigurator#nics' do
      expect(Bosh::HuaweiCloud::NetworkConfigurator).to receive(:new).with(anything, nil).and_call_original
      expect_any_instance_of(Bosh::HuaweiCloud::NetworkConfigurator).to receive(:prepare).with(anything)
      expect_any_instance_of(Bosh::HuaweiCloud::NetworkConfigurator).to receive(:nics).and_return(nics)
    ...
    context 'when vrrp ip is configured' do
      ...
      it 'calls NetworkConfigurator#prepare and NetworkConfigurator#nics' do
        expect(Bosh::HuaweiCloud::NetworkConfigurator).to receive(:new).with(anything, '10.0.0.10').and_call_original
        expect_any_instance_of(Bosh::HuaweiCloud::NetworkConfigurator).to receive(:prepare).with(anything)
        expect_any_instance_of(Bosh::HuaweiCloud::NetworkConfigurator).to receive(:nics).and_return(nics)
    ...
    context 'when vm creation fails' do
      ...
      context 'when NetworkConfigurator#cleanup fails' do
        it 'fails with the vm creation failure' do
          allow_any_instance_of(Bosh::HuaweiCloud::NetworkConfigurator).to receive(:cleanup).and_raise 'BOOM Cleanup!!!'
    ...
    context 'when vm_destroy fails' do
      ...
      it 'calls NetworkConfigurator#cleanup and fails with VMCreationFailed error' do
        allow_any_instance_of(Bosh::HuaweiCloud::NetworkConfigurator).to receive(:configure).and_raise 'BOOM configure!!!'
  ...
  context 'with scheduler hints' do
    ...
    it 'creates an Huawei Cloud server with scheduler hints' do
      allow_any_instance_of(Bosh::HuaweiCloud::NetworkConfigurator).to receive(:configure)
  ...
  context 'when fail to register an Huawei Cloud server after the server is created' do
  ...
  describe 'key_name configuration' do
    ...
    context 'when default_key_name is defined in CPI cloud properties and key_name in resource pool' do
      ...
      it 'takes the key_name from resource pool' do
        expect_any_instance_of(Bosh::HuaweiCloud::VmFactory).to receive(:validate_key_exists).with('test_key')
        cloud.create_vm('agent-id', 'sc-id', resource_pool_spec, { 'network_a' => dynamic_network_spec }, nil, environment)
      end
    end
  ...
  describe 'use "vm-<uuid>" as registry key' do
    ...
    context 'when "human_readable_vm_names" is disabled' do
      ...
      it 'does not tag server with registry tag' do
        ...
        expect(Bosh::HuaweiCloud::TagManager).to_not have_received(:tag_server).with(server, registry_key: "vm-#{unique_name}")
      end
  ...
  describe 'with loadbalancer pool' do
    ...
    it 'creates as many loadbalancers as are listed in the manifest' do
      ...
      pool_membership = Bosh::HuaweiCloud::LoadbalancerConfigurator::LoadbalancerPoolMembership.new('name', 'port', 'pool_id', 'membership_id')

      expect_any_instance_of(Bosh::HuaweiCloud::LoadbalancerConfigurator)
  ...
  describe 'setting VM metadata' do
    context 'when human_readable_vm_names are enabled' do
      ...
      it 'tags registry_key with "vm-<uuid>"' do
        ...
        expect(Bosh::HuaweiCloud::TagManager).to have_received(:tag_server).with(server, registry_key: "vm-#{unique_name}")
      end

      it 'raises an exception, if tagging fails' do
        allow(Bosh::HuaweiCloud::TagManager).to receive(:tag_server).and_raise(StandardError)
    ...
    context 'when loadbalancer_pools are present' do
      ...
      let(:loadbalancer_configurator) { instance_double(Bosh::HuaweiCloud::LoadbalancerConfigurator) }
      ...
      it 'tags the vm with the lbaas pool and membership' do
        ...
        expect(Bosh::HuaweiCloud::TagManager).to have_received(:tag_server).with(server,
                                                                                    'lbaas_pool_1' => 'pool_id/membership_id',
                                                                                    'lbaas_pool_2' => 'pool_id/membership_id')
      end
  ...
  describe 'enable_auto_anti_affinity' do
    ...
    context 'when "enable_auto_anti_affinity" is true' do
      ...
      it 'does not set server group name if bosh group is missing' do
        ...
        expect(Bosh::HuaweiCloud::ServerGroups).to_not have_received(:new)
      end
  ...
  describe 'when multiple azs are configured' do
    ...
    it 'iterates over the azs till it can create a vm in one of the configured azs' do
      allow_any_instance_of(Bosh::HuaweiCloud::AvailabilityZoneProvider).to receive(:select_azs).and_return(azs)
