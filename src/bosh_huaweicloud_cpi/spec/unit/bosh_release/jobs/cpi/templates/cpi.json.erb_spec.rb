...
describe 'cpi.json.erb' do
  let(:cpi_specification_file) { File.absolute_path(File.join(jobs_root, 'huaweicloud_cpi/spec')) }
  ...
  let(:cpi_json_erb) { File.read(File.absolute_path(File.join(jobs_root, 'huaweicloud_cpi/templates/cpi.json.erb'))) }
  let(:manifest) do
    {
      'properties' => {
        'huaweicloud' => {
