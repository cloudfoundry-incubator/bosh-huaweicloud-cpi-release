...
describe Bosh::HuaweiCloud::Cloud do
  before(:each) { allow(Bosh::HuaweiCloud::TagManager).to receive(:tag_server) }
  ...
  context 'without registry_key set' do
    ...
    it 'should only tag with metadata' do
      ...
      expect(Bosh::HuaweiCloud::TagManager).to receive(:tag_server).with(server, job: 'job', index: 'index')
