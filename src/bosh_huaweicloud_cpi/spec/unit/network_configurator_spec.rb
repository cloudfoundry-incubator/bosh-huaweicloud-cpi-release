...
describe Bosh::HuaweiCloud::NetworkConfigurator do
  ...
  describe '#initialize' do
    context 'when spec is not a hash' do
      it 'should raise an error' do
        expect {
          Bosh::HuaweiCloud::NetworkConfigurator.new('foo')
  ...
  describe '#nics' do
    ...
    context 'when manual network' do
      let(:nc) do
        ...
        Bosh::HuaweiCloud::NetworkConfigurator.new(manual_network)
      end
  ...
  describe '#prepare' do
    ...
    before(:each) do
      [Bosh::HuaweiCloud::ManualNetwork, Bosh::HuaweiCloud::DynamicNetwork].each do |class_name|
  ...
  describe '#configure' do
    let(:vip_network) do
      ...
      allow(Bosh::HuaweiCloud::VipNetwork).to receive(:new).and_return(network)
      network
    end
    let(:dynamic_network) do
      network = double('dynamic_network')
      allow(Bosh::HuaweiCloud::DynamicNetwork).to receive(:new).and_return(network)
      network
    end
    let(:manual_network) do
      network = double('manual_network')
      allow(Bosh::HuaweiCloud::ManualNetwork).to receive(:new).and_return(network)
      network
    end
    ...
    context 'With vip network' do
      ...
      it 'configures the vip network and all other networks too' do
        ...
        network_configurator = Bosh::HuaweiCloud::NetworkConfigurator.new(network_spec)
  ...
  describe '.get_gateway_network_id' do
    ...
    context 'multiple network configured' do
      ...
      it 'should return default gateway network id' do
        expect(Bosh::HuaweiCloud::NetworkConfigurator.get_gateway_network_id(network_spec)).to eq('net_id_b')
      end
    end
  end

  describe '.gateway_ip' do
    let(:server) { instance_double(Fog::Compute::HuaweiCloud::Server) }
  ...
  describe '.matching_gateway_subnet_ids_for_ip' do
    let(:neutron) { double(Fog::Network::HuaweiCloud) }
