...
describe Bosh::HuaweiCloud::LoadbalancerConfigurator do
  ...
  let(:server) { instance_double(Fog::Compute::HuaweiCloud::Server, id: 1234) }
  let(:pool) {
    {
      'name' => 'my-lb-pool',
      'port' => 8080,
  ...
  describe '#create_pool_memberships' do
    context 'when a list of loadbalancer_pools is given' do
      ...
      it 'creates the memberships and returns the corresponding server_tags' do
        allow(Bosh::HuaweiCloud::NetworkConfigurator).to receive(:matching_gateway_subnet_ids_for_ip).and_return(['sub-net-id'])
  ...
  describe '#create_membership' do
    ...
    it 'waits for loadbalancer to be active and returns the id of the existing membership' do
      loadbalancer = instance_double(Bosh::HuaweiCloud::LoadbalancerConfigurator::LoadBalancerResource, 'loadbalancer', provisioning_status: 'ACTIVE')
    ...
    context 'when lbaas resource cannot be found' do
      before(:each) do
        allow(subject).to receive(:loadbalancer_id).and_raise(Bosh::HuaweiCloud::LoadbalancerConfigurator::LoadBalancerResource::NotFound, 'some-error-message')
      end
    ...
    context 'when lbaas resources are not in a valid state e.g. pool has more than one loadbalancer associated' do
      before(:each) do
        allow(subject).to receive(:loadbalancer_id).and_raise(Bosh::HuaweiCloud::LoadbalancerConfigurator::LoadBalancerResource::NotSupportedConfiguration, 'some-error-message')
      end
  ...
  describe '#add_vm_to_pool' do
    ...
    context 'when ip address cannot be determined' do
      before(:each) do
        allow(Bosh::HuaweiCloud::NetworkConfigurator).to receive(:gateway_ip).and_raise(Bosh::Clouds::VMCreationFailed.new(false), 'Original message.')
      end
    ...
    context 'when multiple subnet ids match' do
      ...
      it 'errors' do
        allow(Bosh::HuaweiCloud::NetworkConfigurator).to receive(:matching_gateway_subnet_ids_for_ip).and_return(['subnet-id', 'other-subnet-id'])
        expect {
          subject.add_vm_to_pool(server, network_spec, pool)
    ...
    context 'when no subnet id match or no subnet exists' do
      ...
      it 'errors' do
        allow(Bosh::HuaweiCloud::NetworkConfigurator).to receive(:matching_gateway_subnet_ids_for_ip).and_return([])
        expect {
          subject.add_vm_to_pool(server, network_spec, pool)
  ...
  describe '#remove_vm_from_pool' do
    ...
    context 'when load balancer resources are not in a valid state e.g. pool has more than one loadbalancer associated' do
      before(:each) do
        allow(subject).to receive(:loadbalancer_id).and_raise(Bosh::HuaweiCloud::LoadbalancerConfigurator::LoadBalancerResource::NotSupportedConfiguration)
      end
    ...
    context 'when load balancer resource cannot be found' do
      before(:each) do
        allow(subject).to receive(:loadbalancer_id).and_raise(Bosh::HuaweiCloud::LoadbalancerConfigurator::LoadBalancerResource::NotFound)
      end
    ...
    context 'when membership not found' do
      before do
        allow(network).to receive(:delete_lbaas_pool_member).and_raise(Fog::Network::HuaweiCloud::NotFound)
      end
    ...
    context 'when membership deletion fails' do
      it 're-raises as CloudError' do
        allow(network).to receive(:delete_lbaas_pool_member).and_raise(Fog::Network::HuaweiCloud::Error.new('BOOM!!!'))

        expect {
          subject.remove_vm_from_pool(pool_id, membership_id)
        }.to raise_error(Bosh::Clouds::CloudError, "Deleting load balancer pool membership with pool_id 'pool-id' and membership_id 'membership-id' failed. Reason: Fog::Network::HuaweiCloud::Error BOOM!!!")
      end
    end
  ...
  describe '#loadbalancer_id' do
    ...
    context 'when pool exists' do
      ...
      context 'when pool has no loadbalancer associated' do
        ...
        it 'raises an error' do
          ...
          }.to raise_error Bosh::HuaweiCloud::LoadbalancerConfigurator::LoadBalancerResource::NotFound, "No load balancers associated with load balancer pool 'pool-id'"
        end
      end
      ...
      context 'when pool has more than one loadbalancer associated' do
        ...
        it 'raises an error' do
          expect {
            subject.loadbalancer_id(pool_id)
          }.to raise_error(Bosh::HuaweiCloud::LoadbalancerConfigurator::LoadBalancerResource::NotSupportedConfiguration, "More than one load balancer is associated with load balancer pool 'pool-id'. It is not possible to verify the status of the load balancer responsible for the pool membership.")
        end
      end
      ...
      context 'when pool does not have loadbalancer property (before Newton)' do
        ...
        context 'when listener does not exist' do
          ...
          it 'raises an error' do
            expect {
              subject.loadbalancer_id(pool_id)
            }.to raise_error Bosh::HuaweiCloud::LoadbalancerConfigurator::LoadBalancerResource::NotFound, "No listeners associated with load balancer pool 'pool-id'"
          end
        end
        ...
        context 'when multiple listeners do exist' do
          ...
          it 'raises an error' do
            expect {
              subject.loadbalancer_id(pool_id)
            }.to raise_error Bosh::HuaweiCloud::LoadbalancerConfigurator::LoadBalancerResource::NotSupportedConfiguration, "More than one listener is associated with load balancer pool 'pool-id'. It is not possible to verify the status of the load balancer responsible for the pool membership."
          end
        end
      end
    end
    ...
    context 'when pool does not exist' do
      before(:each) do
        allow(network).to receive(:get_lbaas_pool).and_raise(Fog::Network::HuaweiCloud::NotFound.new, 'some-error-message')
      end

      it 'raises an error' do
        expect {
          subject.loadbalancer_id(pool_id)
        }.to raise_error Bosh::HuaweiCloud::LoadbalancerConfigurator::LoadBalancerResource::NotFound, "Load balancer ID could not be determined because pool with ID 'pool-id' was not found. Reason: some-error-message"
      end
    end
  end
